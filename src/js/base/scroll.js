...
!(function ($) {
        ...
        if (this.options.role === 'slider' ) {
            ...
            if (this.options.indicator ) {
                ...
                for (var i=1; i<=this.count; i++) {
                    if (i===1) {
                        temp += '<li class="current" data-index="' +i+ '">'+i+'</li>';
                    } else {
                        temp += '<li data-index="'+i+'">'+i+'</li>';
                    }
                }
                temp += '</ul>';

                var inds = $('.ui-slider-indicators').length;
                if(this.count>0 && !inds) {
     ...
     * _start: _start, _move: _move, _end: _end, beforeScrollStart: _resize,
    ...
    Scroll.prototype = {
        ...
        refresh: function () {
            ...
            var pt = matrix['padding-top'].replace(/[^-\d.]/g, '')||0,
                pb = matrix['padding-bottom'].replace(/[^-\d.]/g, '')||0,
                pl = matrix['padding-left'].replace(/[^-\d.]/g, '')||0,
                pr = matrix['padding-right'].replace(/[^-\d.]/g, '')||0;

            var matrix2 = window.getComputedStyle(this.scroller, null)||0;
            var	mt2 = matrix2['margin-top'].replace(/[^-\d.]/g, '')||0,
                mb2 = matrix2['margin-bottom'].replace(/[^-\d.]/g, '')||0,
                ml2 = matrix2['margin-left'].replace(/[^-\d.]/g, '')||0,
                mr2 = matrix2['margin-right'].replace(/[^-\d.]/g, '')||0;
